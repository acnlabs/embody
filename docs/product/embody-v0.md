...
```text
agent arrives
  → join Embody: the hosted studio mints the body id — origin is written on the body
    (no studio URL → the CLI mints a local-only id that can never push)
...
embody web           hosted owner studio + body registry (Auth0, same tenant as ComicLaw)
                     mints body ids on join. Agent push writes snapshots here.
...
The hosted studio is also the **body registry**: like an agent joins ACN for an `agent_id`, a body joins Embody for its `body_{id}`. `POST /api/agent/bodies` (ACN key) mints the id and records `bound_agent_id` as the joining agent. A body that joined but never pushed still shows in the owner's list — registered, no snapshot yet. `push` only updates joined bodies; an unknown id is refused. Bodies born before the registry are adopted by `embody join --body …`, which registers the existing local id instead of minting a new one.
...
`kind` says how the machine drives — the control contract (observation/action shape, runtime). It does not say what one particular unit carries. A body also has a `build`: the as-built manifest of this one unit — BOM revision, modules the unit actually has (`imu`, `foot_contact`, …), serial, calibration. Sim bodies take the kind's default build, which only records sensors the runtime evidents. Official Microduck policy obs is 61-dim (proprioception + commands); a sim cannot declare `camera`. The real robot lists a front camera; robot units record theirs at pairing. Join records the birth build on the hosted registry; `push` refreshes it. An empty local manifest is uninitialized and is filled with the kind default. v0 records and shows the build (16KiB cap); policy hardware-gating on attach is a later seam, not this probe.
...
| Train | Studio verb; the kind runtime trains (no generic PPO in the kernel) | Runtime only. No trainer in this kernel. microduck-skill `train.sh --embody-as` (or `training start`) points the owner room until attach of that alias, `export_publish`, or `training clear`. |
...
| Show | Card + numbers | CLI `show` / `status`. Agent `push`es a snapshot to hosted owner studio (`web/`). `policy attach` also pushes the card grid (Hub + `curves`) so the room updates without a session. `training start` pushes a Training pointer (not a drive). `session start` detaches `push --watch`: live numbers, pose ~5 Hz, and owner-page `twist` / `do` / `halt` (WASD or pad; last write wins). Canvas drag only orbits. The page cannot reach localhost sim. Hub preview + onnx; live `numbers` pass through. Session verbs push a `control` event (agent); the page appends owner drive. Attach and training do not. Cards link the Hub card; `curves` jumps to wandb when the Hub manifest had a run URL. Training Watch is huggingface.co or wandb.ai. No fallen verdict. Agent replies on ACN. Local `embody studio` is debug only. Interfaze may iframe `/b/{id}/host` (short-lived hostToken, no studio chrome, no nested chat). One body opens directly; many bodies, the owner picks. Interfaze refreshes that ticket before it expires, and again if the tab slept past expiry. If the view expired, opening Body again remints the same body. |
...
| embody web | Hosted owner rooms. Mint body ids (`join`). Auth0 (same tenant as ComicLaw). Accept agent `push`. Enqueue `twist` / `do` / `halt` for the session watch to run. Owner rooms show name, origin, and on/off — not kind. Owner may embed Interfaze chat with the bound agent (`context: body:{id}`) after the origin is in Gateway `embed_hosts`. Interfaze may iframe `/b/{id}/host` with a short-lived hostToken (`POST /api/user/host`); that page has no studio chrome and no nested chat — words stay in Interfaze. One body opens directly; many bodies return a pick list, then Interfaze opens the chosen id. Interfaze renews the ticket before it expires (same body), and again if the tab slept past expiry. If the view expired, opening Body again remints the same body. Credits stay on AgentPlanet chat. | Talk to localhost sim. Hub search. Charge `source=embody`. Localhost-as-product. Nested Interfaze chat on the host page. |
