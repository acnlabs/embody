...
| Show | Card + numbers | CLI `show` / `status`. Agent `push`es a snapshot to hosted owner studio (`web/`). `policy attach` also pushes the card grid (Hub + `curves`) so the room updates without a session. `session start` detaches `push --watch`: live numbers, pose ~5 Hz, and owner-page `twist` / `do` / `halt` (WASD or pad; last write wins). Canvas drag only orbits. The page cannot reach localhost sim. Hub preview + onnx; live `numbers` pass through. Session verbs push a `control` event (agent); the page appends owner drive. Attach does not. Cards link the Hub card; `curves` jumps to wandb when the Hub manifest had a run URL. No fallen verdict. Agent replies on ACN. Local `embody studio` is debug only. |
