...
| Train | Studio verb; the kind runtime trains (no generic PPO in the kernel) | Runtime only. No trainer in this kernel. Agent may `training start` a pointer (`--as`, optional Watch URL) so the owner room shows Training … until attach of that alias or `training clear`. |
